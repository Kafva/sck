...
#		patches/libxft-bgra.patch

[ $(whoami) != root ] && { echo "Run as root"; exit 1 ; }

PROJECT_ROOT=$(realpath $0 | sed "s@/scripts/$(basename $0)@@")

mkdir -p /etc/portage/patches/x11-libs/libXft 
cp $PROJECT_ROOT/patches/libxft-bgra.patch /etc/portage/patches/x11-libs/libXft &&
